...
define({
	name : 'views/main',
	requires : [ 'core/event', 'core/application', 'audios/stream',
			'audios/audio', 'timers/timer', 'helpers/timer' ],
	def : function viewsMain(req) {
		'use strict';

		var e = req.core.event, s = req.audios.stream, a = req.audios.audio, Timer = req.timers.timer.Timer, Time = req.helpers.timer.Time,

		page = null, pageId = 'main', timer = null, initialised = false, startbtn = null, stopbtn = null, pceventbtn_right = null, pceventbtn_left = null, isCircle = tau.support.shape.circle,
		...
		var rightEventTimeArray = new Array(), leftEventTimeArray = new Array();
		// ////////////////////////////////////////////////RECORDING////////////////////////////////////////////////////////////////////////
		...
		function refreshTimer() {
			 ...
			 * @type {array}
			 */
			var time = new Time(timer.getTimeElapsed()), i, element;

			for (i = time.length - 1; i >= 0; i -= 1) {
				element = document.getElementById('d' + i);
				element.classList.remove.apply(element.classList, [ 'd0', 'd1',
		...
		function sendJsonEventTime() {
			...
			try {
				...
				rightEventTimeArray = [];
				leftEventTimeArray = [];
				console.log("eventTimeArray Initialize Success : "
						+ rightEventTimeArray + leftEventTimeArray);
			} catch (err) {
		...
		function rotaryDetentHandler(ev) {
		...
		function bindEvents() {
			...
			if (isCircle)
				document.addEventListener("rotarydetent", rotaryDetentHandler);// rotary event
		}
